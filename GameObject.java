...
public class GameObject {
	...
	public void update() { // Gère le mouvement du GameObject
		view.setTranslateX(view.getTranslateX() + velocity.getX()); // Translation selon les x (si velocity.getX()=0 pas
																	// de mouvement)
		view.setTranslateY(view.getTranslateY() + velocity.getY()); // Translation selon les y (si velocity.getY()=0 pas
																	// de mouvement)

	}

	public void addGameObject(Pane root, double x, double y) { // Ajout d'un sprite "objet" aux coord x et y dans la
																// fenêtre
	...
	public double distance(GameObject other) { // Calcul de la distance entre deux GameObject
		return Math.sqrt(Math.pow(view.getTranslateX() - other.view.getTranslateX(), 2)
				+ Math.pow(view.getTranslateY() - other.view.getTranslateY(), 2));
	}
